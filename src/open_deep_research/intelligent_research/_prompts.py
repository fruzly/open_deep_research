...
def get_research_prompt(
    ...
    """
    Gets the specified type of research prompt.
    ...
    prompt_mapping = {
        ...
        "gap_identification": KNOWLEDGE_GAP_IDENTIFICATION_PROMPT,
        "supervisor_thinking": SUPERVISOR_EXTENDED_THINKING_PROMPT,
        "researcher_thinking": RESEARCHER_INTERLEAVED_THINKING_PROMPT
    }
    
    if prompt_type not in prompt_mapping:
        raise ValueError(f"Unknown prompt type: {prompt_type}")
    
    return format_prompt_with_context(prompt_mapping[prompt_type], context)
...
def evaluate_prompt_effectiveness(
    ...
    """
    Evaluates the effectiveness of a prompt.
    ...
    if expected_format.lower() == "json":
        try:
            import json
            json.loads(response)
            evaluation["format_compliance"] = True
        except:
            evaluation["format_compliance"] = False
            evaluation["suggestions"].append("Response format does not comply with JSON requirements")
